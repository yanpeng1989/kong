...
for _, strategy in helpers.each_strategy() do
  describe("use-cases with DB: #" .. strategy, function()
    ...
    it("retrieves plugins for plugins_iterator", function()
      ...
      local rows, err = db.plugins:select_by_ids("key-auth", nil, service.id, nil, nil)
      assert.is_nil(err)
      assert.equal(1, #rows)
      ...
      rows, err = db.plugins:select_by_ids("rate-limiting", nil, service.id, nil, nil)
      assert.is_nil(err)
      assert.equal(2, #rows)

      -- TEST 3: retrieve rate-limiting plugin for Service + Consumer

      rows, err = db.plugins:select_by_ids("rate-limiting", nil, service.id, consumer.id, nil)
      assert.is_nil(err)
      assert.equal(1, #rows)
