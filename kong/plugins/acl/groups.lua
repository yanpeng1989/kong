...
-- The table will have an array part to iterate over
-- Eg.
-- {
--   [1] = "users",
--   [2] = "admins",
...
local function get_authenticated_groups()
  if type(kong.ctx.shared.authenticated_groups) == "table" then
    return kong.ctx.shared.authenticated_groups
  end

  if type(ngx.ctx.authenticated_groups) == "table" then
    return ngx.ctx.authenticated_groups
  end
end
...
-- @param groups (table) list of groups (result from`get_authenticated_groups`)
...
local function group_in_groups(groups_to_check, groups)
  for i = 1, #groups_to_check do
    for j = 1, #groups do
      if groups_to_check[i] == groups[j] then
        return true
      end
    end
  end
