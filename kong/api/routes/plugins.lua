...
do
  local function field_to_jsonable(field)
    local out = {}
    local name = next(field)
    local attrs = field[name]
  ...
  schema_to_jsonable = function(schema)
    ...
    for _, field in ipairs(schema.fields) do
      table.insert(fields, field_to_jsonable(field))
    end
...
return {
  ...
  ["/apis/:api_name_or_id/plugins/"] = {
    before = function(self, _, helpers)
      crud.find_api_by_name_or_id(self, kong.dao, helpers)
      self.params.api = {id = self.api.id }
    end,

    GET = function(self)
      crud.paginated_set(self, kong.dao.plugins)
    end,

    POST = function(self)
      crud.post(self.params, kong.dao.plugins, function(data)
        local r_data = utils.deep_copy(data)
        r_data.config = nil
        r_data.e = "a"
        reports.send("api", r_data)
